<script lang="ts">
    ...
    async function createPost(){
        ...
        if(title == "" || description == "") return alert("Input can't be empty!");
        try {
...
<div class="w-full flex">
    <Sidebar/>
    <div id="post-preview" class="container h-[100vh] mx-auto overflow-y-scroll pb-10 w-[900px]">
        <div class="grid grid-cols-1 place-items-center">
            ...
            <img class="w-[600px] mt-10 mb-1" src={postImage || "https://picsum.photos/800"} on:click={() => inputFile.click()} role="button">
            <input bind:this={inputFile} on:change={changePostImage} type="file" class="hidden">
            <span class="text-base text-white">
                *klik gambar untuk mengedit
            </span>
            <input bind:value={title} type="text" class="mt-10 w-[600px] bg-[#fff0] p-3 text-white text-lg border border-white outline-none focus:border-[#2d87e2] placeholder:text-gray-400 placeholder:italic" placeholder="Tambahkan judul">
            <textarea class="mt-7 p-4 w-[600px] h-[200px] bg-[#ffffff00] rounded-lg text-white text-lg border border-white outline-none focus:border-[#2d87e2] placeholder:text-gray-400 placeholder:italic"
                bind:value={description} cols="30" rows="10" placeholder="Tambahkan deskripsi"/>
            ...
            <button class="bg-[#0E8388] hover:bg-[#2ec3c8] px-8 py-3 rounded-lg mt-7" on:click={createPost}>Post</button>
        </div>
    </div>
    <Profile/>
</div>
