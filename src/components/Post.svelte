...
<script lang="ts">
    ...
    async function onRil() {
        ...
        try {
            // @ts-ignore
            const rilRes: FetchResult<RilResult> = await sendRil({
    ...
    async function onFek() {
        ...
        try {
            // @ts-ignore
            const fekRes: FetchResult<FekResult> = await sendFek({
                variables: { post_id: post.id },
            });
...
<div
    class="grid grid-cols-1 mx-auto my-10 w-[600px] rounded-xl drop-shadow-2xl bg-[#222]"
>
        ...
        <div class="grid grid-cols-2 place-items-center">
            {#if profile != null}
                ...
                >
                    ...
                    <img
                        src={profile}
                        class="w-[45px] h-[45px] rounded-full border border-white"
                    />
                </a>
            {/if}
            ...
            <a
                href={"/profile" +
                    (username === post.username ? "" : `/${post.username}`)}
                use:link
                ><span class="text-white text-lg">{post.username}</span></a
            >
        </div>
    ...
    <div class="post-action flex flex-row gap-[4px] mt-4 px-4">
        ...
        <div
            class={"text-2xl " + (user_ril ? "text-green-500" : "text-white")}
            on:click={onRil}
            role="button"
            tabindex="0"
        >
        ...
        <div
            class={"text-2xl ms-2 " +
                (user_fek ? "text-red-500" : "text-white")}
            on:click={onFek}
            role="button"
            tabindex="0"
        >
